...
public class FadeEffect : SingletorPersistent<FadeEffect>
{
    ...
    [SerializeField]
    [Range(0f, 0.5f)]
    float m_loadingStepTime=.01f;            // A range of time to wait for every repetition on the effect

    [SerializeField]
    [Range(0f, 0.5f)]
    float m_loadingStepValue =.05f;           // The value to modify the alpha every steo time
